...
if command_line_args:
	...
	env.env.color_list = get_reacher_task_color_info(task_color_index=0)
...
for t in range(env.env.T):
    ...
    rgb_img = env.render()
    if mode != "human":
        img=Image.frombuffer("RGB", rgb_img.shape[:-1], rgb_img.tostring(), 'raw')
        filename = 'frame_' + str(t) + '.png'
