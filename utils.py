    ...
    if type(demo_file) is not list:
        ...
        for i in range(0, len(demo_file)):
            with Timer('Extracting demo file %d' % i):
                ...
                # Discard Joint velocities and end effector position and retain only joint pose
                demos[i]['demoX'] = demos[i]['demoX'][:,:,0:2]
                demos[i]['demoU'] = demos[i]['demoU'][:,:,0:2]
    return demos

def extract_rbc_demo_dict(demo_files, n_folders, n_examples_per_folder):
    demos = {}
    for key in range(n_folders):
      for i in range(n_examples_per_folder):
        ...
        with Timer('Extracting demo file %d' % i):
          ...
          f.close()
      demos[key] = demo
      with open('/content/drive/My Drive/Code/mil/data/vision_reach/color_data/demos_' + str(key) + '.pkl' , 'wb') as f:
